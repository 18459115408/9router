...
## Removed
- **npm 自升级机制（个人 fork 模式）**：本仓库升级走 `git merge upstream/master`，npm registry 版本对比只会把上游发行版包装进来，故整条链路移除——侧边栏的 “New version available / 立即更新” 横幅与手动更新面板、`GET /api/version`（hasUpdate 检查）、`POST /api/version/update`（会 `npx 9router` 拉起 npm 安装的死接口）、`src/lib/updater/updater.js`（detached updater 脚本）、`appUpdater.js` 的 spawn 半边（kill 半边保留，移至 `src/lib/processKill.js`）、CLI 启动时的 `checkForUpdate()` 与 “Update to vX” 菜单项及 `--skip-update` 参数、`UPDATER_CONFIG` 的 install 系字段（`appPort` 并入 `APP_CONFIG`）。保留：`POST /api/version/shutdown`（profile 页与 HeaderMenu 的关停按钮在用）、侧边栏版本号展示。合并上游后若这些文件复活，按 `docs/UPSTREAM-SYNC.md` §2.1 再删。

## Features
- **Baidu Netdisk Sync UI**: the feature is now reachable from the dashboard — a status card under Settings (`/dashboard/profile`) shows scheduler / authorization / last-sync / last-error state with a **Sync Now** button (`POST /api/sync/baidu/trigger`), an authorize / re-authorize link, and a field to paste the oob authorization code so the flow never requires hand-editing a URL (`/api/sync/baidu/exchange` gained `?format=json` for this). zh-CN strings added to `public/i18n/literals/zh-CN.json`. Operational notes and the known issues found while investigating (silent tick failures, plaintext token + no-op `0600` on Windows, `DATA_DIR` Unix-path fallback, per-machine authorization, fresh-instance pull-overwrites-local) are recorded in `docs/BAIDU-SYNC.md`.
...
## Fixes
- **Baidu Netdisk Sync (upload path)**: the sync could never push — it used the single-step `method=upload` endpoint, which Baidu has retired (`pcs/file` → 31064 "file is not authorized", `xpan/file` → 31832 "unsupported api"), while the working chunked flow was gated behind a >2GB size check. `uploadSingleStep()` and `SINGLE_STEP_MAX` are gone; every upload now goes `precreate → superfile2 → create`. Verified end-to-end: the first real push produced a 338933-byte `data.sqlite.enc` on the netdisk matching `pushedBytes`.
- **Baidu Netdisk Sync**: `GET /api/sync/baidu/status` reported `schedulerStarted: false` while the scheduler was in fact running — the running flag was module-local, and Next does not share a module instance between `instrumentation.js` (which starts the scheduler) and a route handler (which reads the flag). The flag is now mirrored on the process global, which also stops `custom-server.js` and `instrumentation.js` from starting two timers in one process and double-ticking the sync.
